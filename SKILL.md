...
description: 将知网 CAJ / KDH / NH 文献高保真转换为 PDF（保留文字层可选中 + 目录书签）。当用户要转换 .caj 文件、说"caj转pdf""知网文献转pdf""把caj转成pdf""转换知网文献"使用时。技能为自包含离线版：已内置修复好的 caj2pdf 源码 + 闭源解码 DLL + PyMuPDF 修复，AI 可一键批量代转；失败文件给全球学术快报官方无损兜底。
---
...
| 自包含离线 | 技能目录内已带齐源码 + DLL + 依赖，无需联网即可运行 |
...
| DLL | Windows 上 caj2pdf 依赖的闭源解码器文件（已内置） |
...
## 技能结构（自包含，无需联网）
```
~/.workbuddy/skills/caj-to-pdf/
├── SKILL.md
└── scripts/
    ├── setup.py              # 自动建隔离 venv + 装依赖（幂等）
    ├── convert.py            # 批量转换驱动（自动定位 venv 并 re-exec）
    └── caj2pdf-restructured/ # 已修复源码（含双路径真 DLL + Python3.13 补丁）
```
- DLL 已内置在 `caj2pdf-restructured/caj2pdf/bin/` 与 `caj2pdf-restructured/caj2pdf/dep/bin/`（KDH 与 HN 解码走不同包路径，必须两处都有）。
...
2. **运行转换**（二选一）：
   - 单文件：`python ~/.workbuddy/skills/caj-to-pdf/scripts/convert.py "C:/路径/某文献.caj"`
   - 批量：`python ~/.workbuddy/skills/caj-to-pdf/scripts/convert.py --indir "目录" --outdir "C:/tmp/converted" -r`（`-r` 递归；`--outdir` 省略则各文件同目录）
3. **检查输出**：确认每个输入生成同名 `.pdf`；退出码 0=全成功，1=有失败。
4. **处理失败**：见下方「失败回退」。

- 仅建环境（可选）：`python ~/.workbuddy/skills/caj-to-pdf/scripts/setup.py`
...
4. **DLL 必须两处**：`caj2pdf/bin/`（KDH）与 `caj2pdf/dep/bin/`（HN），否则 HN 报找不到 DLL。
...
## 安全与限制
- 仅本地转换，不上传任何文件。
- 解码依赖上游 caj2pdf 的**闭源 DLL**（Windows 预编译，已内置）；Linux/macOS 需自行编译对应 so。
